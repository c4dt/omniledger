...
import Shake from 'keccak';
import Keccak from 'keccak';
import {Log} from './Log';
import {Point, curve} from '@dedis/kyber';
import {DecodeKey, CalypsoWriteInstance, Write, CalypsoReadInstance} from './cothority/calypso/calypso-instance';
...
describe('In a full byzcoin setting, it should', () => {
  ...
  beforeAll(async () => {
    ...
    for ( let i = 0; i < roster.list.length; i++){
      Log.lvl2("Authorizing byzcoin on node", roster.list[i]);
  ...
  it('be able to create an LTS', async () => {
    ...
    let decrypt = await ocs.decryptKey(await tdAdmin.bc.getProof(wrInst.id), await tdAdmin.bc.getProof(readInst.id));
