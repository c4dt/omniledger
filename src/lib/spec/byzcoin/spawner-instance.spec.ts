...
describe("SpawnerInstance Tests", () => {
    ...
    it("should spawn a pop party", async () => {
        ...
        const darcOrg = await spawnUserDarc(si, ci, [SIGNER], org.public, "org");
        ...
        const darcOrg2 = await spawnUserDarc(si, ci, [SIGNER], org2.public, "org2");
        ...
        const darcAtt = await spawnUserDarc(si, ci, [SIGNER], attendee.public, "attendee");
    ...
    it("should not try to spawn existing instances", async () => {
        ...
        const userDarc = await spawnUserDarc(si, ci, [SIGNER], user.public, "org");
        await expectAsync(spawnUserDarc(si, ci, [SIGNER], user.public, "org")).toBeRejected();
...
async function spawnUserDarc(si: SpawnerInstance, ci: CoinInstance, signers: ISigner[], pub: Point, alias: string):
    Promise<DarcInstance> {
    const d = Contact.prepareUserDarc(pub, alias);
