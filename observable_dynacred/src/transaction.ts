...
import {Darc, IdentityDarc, IIdentity, SignerEd25519} from "@dedis/cothority/darc";
import Long from "long";
import {
    Coin,
    CoinInstance,
    CredentialsInstance, CredentialStruct,
    DarcInstance, SPAWNER_COIN,
    SpawnerInstance
...
export class Transaction {
    ...
    constructor(private bc: ByzCoinRPC, private spawner: SpawnerInstance, private coin: CoinInstance,
                private darcSigner: IdentityDarc, private signer: SignerEd25519) {
    }

    public async send(wait = 0): Promise<AddTxResponse> {
        const coinFetch = Instruction.createInvoke(this.coin.id,
        ...
        await ctx.updateCountersAndSign(this.bc, [[this.signer]]);
    ...
    public spawnDarcBasic(desc: string, signers: IIdentity[] = [this.darcSigner]): Darc {
    ...
    public createUser(user: UserFactory, initial = Long.fromNumber(0)){
